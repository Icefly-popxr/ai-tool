...
# Detect explicit <at user_id="xxx">name</at> tags inserted by scripts/cron jobs.
# These require building a proper Feishu post payload with text+at elements.
...
def _build_at_post_payload(content: str) -> str:
    """Build Feishu post payload with proper <at> elements for @mentions.

    Takes a string containing <at user_id="xxx">name</at> tags and converts
    them to Feishu post-format at elements. All other text is wrapped in
    text (unstyled) tags. Returns the full JSON payload for msg_type='post'.
    """
    import re as _re
    parts = _re.split(_AT_TAG_RE, content)
    row: list[dict[str, str]] = []
    i = 0
    while i < len(parts):
        if i + 2 < len(parts) and _AT_TAG_RE.match(f'<at user_id="{parts[i+1]}">{parts[i+2]}</at>'):
            # Leading text before the at tag
            text = parts[i].strip()
            if text:
                row.append({"tag": "text", "text": text})
            row.append({"tag": "at", "user_id": parts[i+1], "user_name": parts[i+2]})
            i += 3
        else:
            text = parts[i].strip()
            if text:
            ...
            i += 1
    if not row:
        row.append({"tag": "text", "text": content})
    return json.dumps({"zh_cn": {"content": [row]}}, ensure_ascii=False)
...
def _detect_title_scenario(content: str, name: str, role: str) -> str:
    """根据内容自动检测场景，生成动态卡片标题
    ...
    # 告警/紧急场景
    _alert_keywords = (
        "告警", "alert", "紧急", "error", "failure", "失败", "异常",
        "critical", "故障", "宕机", "崩溃", "危险"
    )
    if any(kw in _content_lower for kw in _alert_keywords):
        _match_count = sum(1 for kw in _alert_keywords if kw in _content_lower)
        if _match_count >= 2:
            return f"🚨 {name}：告警通知"

    # 数据/表格场景（包含表格或明显的数据标记）
    if re.search(r'\|.+\|', content) and re.search(r'[📊📈📉]', content):
        return f"📊 {name}：数据报告"
    if "```" in content and any(kw in content for kw in ("json", "yaml", "代码", "config", "配置")):
        return f"🔧 {name}：配置更新"
    if "```" in content:
        return f"💻 {name}：代码输出"

    # 链接分享场景
    if re.search(r'https?://[^\s]{10,}', content):
        return f"📎 {name}：信息分享"

    # 列表/步骤场景
    if content.count("\n") >= 3 and any(content.lstrip().startswith(c) for c in ("1.", "2.", "- ", "•", "✅")):
        return f"📋 {name}：操作指南"
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Typewriter card reuse: track the active card's message_id within a
        # conversation turn so intermediate tool-call results PATCH the same
        # card instead of spawning new ones.
        self._active_typewriter_msg_id: Optional[str] = None
        self._active_typewriter_chat_id: Optional[str] = None
        # Tool calls buffer: accumulate tool call results within a turn
        # to display them as collapsible sections in a single card.
        self._tool_calls_buffer: List[Dict[str, str]] = []  # [{"name": ..., "content": ...}]
        # CardKit streaming state: track active card_id for in-place streaming updates
        self._active_cardkit_card_id: Optional[str] = None
        self._active_cardkit_chat_id: Optional[str] = None
        self._active_cardkit_msg_id: Optional[str] = None  # message_id of the card in chat
        self._cardkit_streaming_seq: int = 0  # incremental sequence for streaming API
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # 预填充群聊名称缓存（在主线程中完成，避免子线程 token 问题）
            try:
                await self._hydrate_chat_info_cache()
            except Exception as _e:
                logger.debug("[Feishu] Chat info cache preload skipped: %s", _e)
            return True
    ...
    async def send(
        ...
        # ── <at> tag 快捷路径：直接转 Post 格式发送，绕过卡片中间件 ──
        # 卡片中间件会把 <at> 标签当普通文本处理导致发送失败，
        # 必须在进入 typewriter 之前拦截并走 Post 格式。
        if _AT_TAG_RE.search(content):
            try:
                msg_type, payload = self._build_outbound_payload(content)
                chunks = self.truncate_message(payload, self.MAX_MESSAGE_LENGTH)
                for chunk in chunks:
                    response = await self._feishu_send_with_retry(
                        chat_id=chat_id, msg_type=msg_type, payload=chunk,
                        reply_to=reply_to, metadata=metadata,
                    )
                    result = self._finalize_send_result(response, "at-tag send failed")
                    if not result.success:
                        return result
                return SendResult(success=True)
            except Exception as e:
                logger.warning("[Feishu] at-tag fast path failed, falling through: %s", e)

        # ── 飞书卡片打字机效果：原生 SDK 发送，不走 lark-cli ──
        try:
            import asyncio
            def _run_typewriter():
                # 在发送前，如果存在 active_cardkit_msg_id 并与当前 message_id 一致，则直接返回成功
                if self._active_cardkit_msg_id and self._active_cardkit_msg_id == message_id:
                    return SendResult(success=True)
                from datetime import datetime as _dt
                import re as _re
                import time as _time
                if _answer_idx < len(_body_els):
                    _body_els[_answer_idx]["element_id"] = "answer_md"
                _ck_card_id = self._cardkit_create_card(_ck_card)
                if _ck_card_id:
                    _ck_msg_id = self._cardkit_send_to_chat(chat_id, _ck_card_id)
                    if _ck_msg_id:
                        self._active_cardkit_card_id = _ck_card_id
                        self._active_cardkit_chat_id = chat_id
                        self._active_cardkit_msg_id = _ck_msg_id
                        self._active_typewriter_msg_id = _ck_msg_id
                        self._active_typewriter_chat_id = chat_id
                        self._cardkit_streaming_seq = 1
                        self._tool_calls_buffer.clear()
                        return _ck_msg_id
                # cardkit 失败，回退到原生
                msg_id = self._native_send_card(chat_id, _final_card)

                self._tool_calls_buffer = []
                if msg_id:
                    self._active_typewriter_msg_id = msg_id
                    self._active_typewriter_chat_id = chat_id
                return msg_id

            # 在线程池中执行同步操作
            loop = asyncio.get_event_loop()
            msg_id = await loop.run_in_executor(None, _run_typewriter)

            if msg_id:
                return SendResult(success=True)
        except Exception as e:
            logger.warning(f"[Feishu] Typewriter failed: {e}", exc_info=True)
            pass  # 降级到原始发送逻辑
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 飞书 Chat 对象字段是 `type`（不是 `chat_type`），
            # chat_type 来自 chat.get 的额外方法调用。
            # 先读 `type`，没有再回退 `chat_type`，再回退从 `chat_mode` 推断
            raw_chat_type = (
                str(getattr(data, "type", "") or "")
                or str(getattr(data, "chat_type", "") or "")
            ).strip().lower()
    ...
    async def _handle_message_event_data(self, data: Any) -> None:
        ...
        _sid = getattr(getattr(sender, "sender_id", None), "open_id", "?")
        _stype = getattr(sender, "sender_type", "?")
        _mid = getattr(message, "message_id", "?")
        logger.info("[Feishu] RAW inbound event: sender_type=%s sender_id=%s msg_id=%s", _stype, _sid, _mid)
    ...
    async def _handle_message_with_guards(self, event: MessageEvent) -> None:
        ...
        """
        # 新用户消息 → 清除上一轮的卡片追踪，让本轮创建新卡片
        self._active_typewriter_msg_id = None
        self._active_typewriter_chat_id = None
        # CardKit 流式：关闭之前的流式（如果有），然后重置状态
        if self._active_cardkit_card_id:
            self._cardkit_close_streaming(self._active_cardkit_card_id, self._cardkit_streaming_seq)
        self._cardkit_reset_state()
    ...
    def _map_chat_type(raw_chat_type: str) -> str:
        ...
        # Chat.type 字段：private = 单聊, public = 群聊
        if normalized in ("p2p", "private", "dm", "single"):
            return "dm"
        if normalized in ("public", "group"):
            return "group"
    ...
    def _admit(self, sender: Any, message: Any) -> Optional[RejectReason]:
        ...
        # Bots already cleared FEISHU_ALLOW_BOTS gate above — don't
        # double-gate them with require_mention (bot sends plain text
        # "@Name" which lacks the structured <at> tag the mention
        # detector needs, creating a chicken-and-egg problem).
        if require_mention and not is_bot and not self._mentions_self(message):
            return "group_policy_rejected"
        return None
    ...
    async def _hydrate_chat_info_cache(self) -> None:
        """预填充群聊名称缓存，避免子线程中调用飞书 API 失败。

        在主线程连接成功后调用，使用 lark-cli 获取群聊名称并缓存。
        """
        import subprocess
        try:
            result = subprocess.run(
                ["lark-cli", "im", "+chat-list", "--format", "json", "--page-size", "100", "--as", "bot"],
            ...
            if result.returncode == 0:
                ...
                for chat in chats:
                    ...
                    name = chat.get("name", "")
                    if chat_id and name:
                        self._chat_info_cache[chat_id] = {"name": name, "type": "group"}
                logger.info("[Feishu] Preloaded %d chat names into cache", len(chats))
            else:
                logger.warning("[Feishu] Failed to preload chat names: %s", result.stderr[:200])
        except Exception as e:
            logger.warning("[Feishu] Chat info cache preload failed: %s", e)
    ...
    def _build_tool_progress_card(self, card_title: str, card_color: str, footer: dict, display_name: str) -> Optional[dict]:
        """构建工具调用进度卡片"""
        if not self._tool_calls_buffer:
            return None

        _elements = []
        _tool_count = len(self._tool_calls_buffer)
        
        # 获取当前执行的工具名称
        _current_tool = self._tool_calls_buffer[-1].get("name", "工具调用") if self._tool_calls_buffer else "工具调用"
        
        # 折叠的工具调用区域（飞书不支持collapsible块，用markdown模拟）
        _tc_lines = [f"🔧 **工具调用 ({_tool_count}) - 当前: {_current_tool}**"]
        for tc in self._tool_calls_buffer:
            _tc_lines.append(f"  • {tc['content'][:80]}")
        _elements.append({"tag": "markdown", "content": "\n".join(_tc_lines)})

        # 进度提示 - 显示已执行步骤和当前工具
        _progress_text = "🔄 **正在执行**: " + _current_tool + chr(10)
        _progress_text += "📊 **已执行**: " + str(_tool_count) + " 个工具调用" + chr(10)
        _progress_text += "*⏳ 请稍候...*"
        _elements.append({"tag": "markdown", "content": _progress_text})

        # Footer 不加在进度卡片上——进度卡片是临时的，footer 只在最终响应加
    ...
    # ══════════════════════════════════════════════
    # CardKit 流式卡片协议（一张卡片原地更新，无撤回提示）
    # ══════════════════════════════════════════════

    def _cardkit_create_card(self, card: dict) -> Optional[str]:
        """创建 CardKit 实体，返回 card_id。同步调用。"""
        if not self._client:
            return None
        try:
            ...
            resp = self._client.cardkit.v1.card.create(request)
            if resp.data and resp.data.card_id:
                logger.warning("[Feishu] cardkit_create OK: card_id=%s", str(resp.data.card_id)[:20])
                return str(resp.data.card_id)
            logger.warning("[Feishu] cardkit_create failed: code=%s msg=%s",
                           getattr(resp, "code", "?"), getattr(resp, "msg", "?"))
            return None
        except Exception as e:
            logger.warning("[Feishu] cardkit_create exception: %s", e, exc_info=True)
            return None

    def _cardkit_send_to_chat(self, chat_id: str, card_id: str) -> Optional[str]:
        """发送 CardKit 卡片到聊天，返回 message_id。同步调用。"""
        if not self._client:
            return None
        try:
            ...
            logger.warning("[Feishu] cardkit_send_to_chat failed: code=%s",
                           getattr(resp, "code", "?"))
            return None
        except Exception as e:
            logger.warning("[Feishu] cardkit_send_to_chat exception: %s", e)
            return None

    def _cardkit_stream_element(self, card_id: str, element_id: str,
                                content: str, sequence: int = 0) -> bool:
        """流式更新 CardKit 卡片内指定 element 的内容。同步调用。"""
        if not self._client:
            return False
        try:
            body = (
            ...
            resp = self._client.cardkit.v1.card_element.content(request)
            ok = bool(resp.data is not None and getattr(resp, "code", -1) == 0)
            if ok:
                logger.debug("[Feishu] cardkit_stream_element OK: el=%s seq=%d len=%d",
                             element_id[:12], sequence, len(content))
            else:
                logger.warning("[Feishu] cardkit_stream_element failed: card=%s code=%s",
                               card_id[:12], getattr(resp, "code", "?"))
            return ok
        except Exception as e:
            logger.warning("[Feishu] cardkit_stream_element exception: %s", e)
            return False

    def _cardkit_close_streaming(self, card_id: str, sequence: int = 0) -> bool:
        """关闭 CardKit 卡片的流式模式。同步调用。"""
        if not self._client:
            return False
        try:
            ...
            resp = self._client.cardkit.v1.card.settings(request)
            ok = bool(resp.data is not None and getattr(resp, "code", -1) == 0)
            if ok:
                logger.warning("[Feishu] cardkit_close_streaming OK: card=%s", card_id[:16])
            return ok
        except Exception as e:
            logger.warning("[Feishu] cardkit_close_streaming exception: %s", e)
            return False

    def _cardkit_reset_state(self) -> None:
        """重置 CardKit 流式状态"""
        self._active_cardkit_card_id = None
        self._active_cardkit_chat_id = None
        self._active_cardkit_msg_id = None
        self._cardkit_streaming_seq = 0

    # ══════════════════════════════════════════════
    # 原生卡片发送（非流式）
    # ══════════════════════════════════════════════

    def _native_send_card(self, chat_id: str, card: dict) -> Optional[str]:
        """用当前 bot 的 SDK 凭证发送卡片，返回 message_id。同步调用。"""
        if not self._client:
            return None
        try:
            card_json = json.dumps(card, ensure_ascii=False)
            ...
            body = self._build_create_message_body(
                ...
                content=card_json, uuid_value=str(uuid.uuid4()),
            )
            request = self._build_create_message_request(receive_id_type, body)
            response = self._client.im.v1.message.create(request)
            if self._response_succeeded(response):
                _mid = self._extract_response_field(response, "message_id")
                logger.warning("[Feishu] native_send_card OK: msg_id=%s chat=%s", _mid, chat_id[:20])
                return _mid
            logger.warning("[Feishu] native_send_card failed: %s", getattr(response, "msg", "unknown"))
            return None
        except Exception as e:
            logger.warning("[Feishu] native_send_card error: %s", e)
            return None

    def _native_patch_card(self, message_id: str, card: dict) -> bool:
        """用当前 bot 的 SDK 凭证更新卡片。

        策略：先尝试 update API（无撤回通知），失败则回退到 delete+resend。
        """
        if not self._client:
            logger.warning("[Feishu] native_patch_card: _client is None!")
            return False
        try:
            card_json = json.dumps(card, ensure_ascii=False)

            # ── 策略1：尝试 update API（不会产生撤回通知）──
            try:
                ...
                resp = self._client.im.v1.message.update(request)
                if self._response_succeeded(resp):
                    logger.warning("[Feishu] native_patch_card OK (update): msg=%s",
                                   message_id[:30])
                    return True
                logger.warning("[Feishu] native_patch_card update failed (code=%s), falling back to delete+resend",
                               getattr(resp, "code", "?"))
            except Exception as update_e:
                logger.warning("[Feishu] native_patch_card update exception, falling back: %s", update_e)

            # ── 策略2：回退 delete+resend ──
            try:
                from lark_oapi.api.im.v1 import DeleteMessageRequest
                del_req = DeleteMessageRequest.builder().message_id(message_id).build()
                del_resp = self._client.im.v1.message.delete(del_req)
                if not self._response_succeeded(del_resp):
                    logger.warning("[Feishu] native_patch_card: delete failed code=%s msg=%s",
                                   getattr(del_resp, "code", "?"), getattr(del_resp, "msg", "?"))
            except Exception as del_e:
                logger.warning("[Feishu] native_patch_card: delete exception: %s", del_e)

            receive_id = self._active_typewriter_chat_id or ""
            receive_id_type = "chat_id"
            ...
            response = self._client.im.v1.message.create(request)
            if self._response_succeeded(response):
                _new_id = self._extract_response_field(response, "message_id")
                logger.warning("[Feishu] native_patch_card OK (replace): old=%s new=%s",
                               message_id[:30], (_new_id or "?")[:30])
                if _new_id:
                    self._active_typewriter_msg_id = _new_id
                return True
            _code = getattr(response, "code", "?")
            _msg = getattr(response, "msg", "unknown")
            logger.warning("[Feishu] native_patch_card failed: code=%s msg=%s", _code, _msg)
            return False
        except Exception as e:
            logger.warning("[Feishu] native_patch_card EXCEPTION: %s", e, exc_info=True)
            return False

    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        # Check for explicit <at> tags first - these need a post payload
        # with proper Feishu at elements for real @mentions.
        if _AT_TAG_RE.search(content):
            return "post", _build_at_post_payload(content)
